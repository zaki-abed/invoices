...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">الفواتير</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ @yield('title')</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
...
@section('content')
    ...
    {{-- Modal Unarchive Invoice --}}
    <div class="modal fade" id="invoice_unarchive" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form action="{{ route('unarchive-invoice') }}" method="post">
                    @csrf
                    <div class="modal-body">
                        <label for="">هل أنت متأكد من إلغاء أرشفة الفاتورة؟</label>
                        <input type="hidden" name="invoice_id" id="invoice_id" value="">
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">إلغاء</button>
    ...
    @endif
    {{-- Unarchived Invoice --}}
    @if(session()->has('restored_invoice'))
        <script>
            window.onload = function(){
                notif({
                    msg: "تم إلغاء أرشفة الفاتورة بنجاح",
                    type: "success"
                })
            }
        </script>
            ...
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <div class="col-sm-6 col-md-3 mg-t-10 mg-sm-t-0"><a href="{{ route('invoices.create') }}"><button
                                    class="btn btn-primary btn-with-icon btn-block"><i class="fa fa-solid fa-plus"></i>
                                    إضافة فواتير</button></a></div>
                    </div>
                </div>
                <div class="card-body">
...
@section('js')
    ...
    <script>
        ...
        $('#invoice_unarchive').on('show.bs.modal', function(event) {
        var a = $(event.relatedTarget)
        var invoice_id = a.data('invoice_id')
        var modal = $(this)
        modal.find('.modal-body #invoice_id').val(invoice_id);
